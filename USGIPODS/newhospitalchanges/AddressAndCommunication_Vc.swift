//
//  AddressAndCommunication_Vc.swift
//  UniversalSompo
//
...
class AddressAndCommunication_Vc: BaseViewController,UITextFieldDelegate {
...
//        self.totallPremium = UserDefaults.standard.string(forKey: "totallPremium")!
    ...
    private func verifyOTPValidation() -> Bool {
        if !CommonMethods.isInternetAvailable() {
            showWarning("Error", AlertMessage.noInternet.rawValue)
            return false
        }else if (pincode_Txt.text == "") {
            showWarning("Error", AlertMessage.enterPinCode.rawValue)
            return false
        }else if (pincode_Txt.text?.count ?? 0 < 6) {
            showWarning("Error", AlertMessage.invalidPinCount.rawValue)
            return false
        }else if (pincode_Txt.text?.count ?? 0 > 6) {
            showWarning("Error", AlertMessage.invalidPinCount.rawValue)
            return false
        }else if(!CommonMethods.isValidNumericNumber(phoneNumber: pincode_Txt.text!)) {
            showWarning("Error", AlertMessage.invalidPinCodeFormat.rawValue)
            return false
        }
        return true
    }
    ...
    @IBAction func continueAction(){
        self.validation()
        self.saveData()
            let nextvc = UIStoryboard.init(name: "Health", bundle: nil).instantiateViewController(withIdentifier: "MedicalHistory_Vc") as! MedicalHistory_Vc
...
//MARK: -  func
extension AddressAndCommunication_Vc{
